...
#   e.g.: to deploy the silver blueprint use "sh up.sh silver silver"
#
...
if [[ "$#" -ne 2  && "$#" -ne 3 ]]; then
   ...
   echo '   or  : sh up.sh silver silver jamm,pamm'
   exit
fi
...
echo "JAMM $1 build process complete: it took $(($diff / 60)) minutes and $(($diff % 60)) seconds."
